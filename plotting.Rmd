---
title: "plotting"
author: "Maya Sollen-Norrlin"
date: "2023-02-22"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```



## Load libraries
```{r}
library(tidyverse)
library(gridExtra)

```

## Read in data
```{r}

tidyData <- read.csv("chemistry-data-tidy.csv")

# Read in the same data but with nutrients for the dry treatment reported as mg/kg wet weight
dryData <- read.csv("chemistry-data-tidy-dryCorrected.csv")

```

```{r}
str(dryData)

```

```{r}

str(tidyData)

```

## Convert weeks to factor so it can be used to group the plots
```{r}
tidyData$Weeks <- as.factor(tidyData$Weeks)
glimpse(tidyData)
```

```{r}
dryData$Weeks <- as.factor(dryData$Weeks)
str(dryData)
```


## Renaming columns so they're easier to write.
```{r}

tidyData<- tidyData %>% 
  rename(
   "NH4 (mg/kg)" = NH4..mg.kg.,
   "K (mg/kg)" = K..mg.kg.,
   "NO2 (mg/kg)" = NO2..mg.kg.,
   "NO3 (mg/kg)" = NO3..mg.kg.,
   "Moisture (%)" = Moisture....
  )

head(tidyData)

```

```{r}

dryData<- dryData %>% 
  rename(
   "NH4 (mg/kg)" = NH4..mg.kg.,
   "K (mg/kg)" = K..mg.kg.,
   "NO2 (mg/kg)" = NO2..mg.kg.,
   "NO3 (mg/kg)" = NO3..mg.kg.,
   "Moisture (%)" = Moisture....
  )

head(dryData)

```
## Replacing negative values in the moisture and nitrate columns with 0. 
```{r}

dryData <- dryData %>% 
  mutate(across(c(`Moisture (%)`, `NO3 (mg/kg)`:`NH4 (mg/kg)`), ~ ifelse(.x<0, 0, .x)))


tail(dryData, 20)
```


## Make frequency historgrams
```{r}

for (i in 3:7) {
  plot1 <- ggplot(tidyData, aes(tidyData[,i])) +
    geom_histogram() +
    labs(x = colnames(tidyData[i]))
  print(plot1)
  
}
```
Same but with the dry-corrected data
```{r}

for (i in 3:7) {
  plot2 <- ggplot(dryData, aes(dryData[,i])) +
    geom_histogram() +
    labs(x = colnames(dryData[i]))
  print(plot2)
  
}
```

Make boxplots
```{r}

for(i in 4:7){
  plot <- ggplot(tidyData, aes(x = Weeks, y = tidyData[,i])) +
          stat_boxplot(aes(Weeks, tidyData[,i]), geom = "errorbar") +
          geom_boxplot(aes(Weeks, tidyData[,i]), outlier.shape = NA, coef = 0) +
          ylab(colnames(tidyData[i])) +
          facet_wrap(.~Treatment) +
          theme(text = element_text(family = "Arial", size = 16), panel.background = element_rect(fill = NA, colour = 'black')) +
          scale_y_continuous(limits = c(min(tidyData[,i]), max(tidyData[,i]))) 
 print(plot)
 #ggsave(file = paste0("boxplot-by-treatment-", colnames(tidyData[i]), ".jpeg"), plot = plot, height = 7, width = 9, units = "in")
}


```
Same with the dry-corrected data
```{r}

for(i in 4:7){
  plot <- ggplot(dryData, aes(x = Weeks, y = dryData[,i])) +
          stat_boxplot(aes(Weeks, dryData[,i]), geom = "errorbar") +
          geom_boxplot(aes(Weeks, dryData[,i]), outlier.shape = NA, coef = 0) +
          ylab(colnames(dryData[i])) +
          facet_wrap(.~Treatment) +
          theme(text = element_text(family = "Arial", size = 16), panel.background = element_rect(fill = NA, colour = 'black')) +
          scale_y_continuous(limits = c(min(dryData[,i]), max(dryData[,i]))) 
 print(plot)
 #ggsave(file = paste0("boxplot-by-treatment-", colnames(tidyData[i]), ".jpeg"), plot = plot, height = 7, width = 9, units = "in")
}


```


Plotting pH separately so that y axis can be adjusted in order to remove white space on plot
```{r}

 pH_plot <- ggplot(tidyData, aes(x = Weeks, y = pH)) +
          stat_boxplot(aes(Weeks, pH), geom = "errorbar") +
          geom_boxplot(aes(Weeks, pH), outlier.shape = NA, coef = 0) +
          ylab("pH") +
          facet_wrap(.~Treatment) +
          theme(text = element_text(family = "Arial", size = 16), 
                panel.background = element_rect(fill = NA, colour ='black')) +
          scale_y_continuous(limits = c(5.3, max(tidyData$pH)))
 print(pH_plot)

#ggsave(file = "pH-boxplot.jpeg", pH_plot, height = 7, width = 9, units = "in")


```

And with the dry corrected
```{r}

 pH_plot2 <- ggplot(dryData, aes(x = Weeks, y = pH)) +
          stat_boxplot(aes(Weeks, pH), geom = "errorbar") +
          geom_boxplot(aes(Weeks, pH), outlier.shape = NA, coef = 0) +
          ylab("pH") +
          facet_wrap(.~Treatment) +
          theme(text = element_text(family = "Arial", size = 16), 
                panel.background = element_rect(fill = NA, colour ='black')) +
          scale_y_continuous(limits = c(5.3, max(dryData$pH)))
 print(pH_plot2)

#ggsave(file = "pH-boxplot.jpeg", pH_plot, height = 7, width = 9, units = "in")


```


## Plotting Nitrogen line graph (fig. 7)
Select only nitrogen data and assign to new dataframe
```{r}
N_data <- dryData %>%
  select(Sample.ID, `NO3 (mg/kg)`, `NO2 (mg/kg)`, `NH4 (mg/kg)`, Treatment, Weeks)

head(N_data)
```

```{r}

N_data[!complete.cases(N_data), ]

```

```{r}

N_data <- N_data %>%
  drop_na()

N_data[!complete.cases(N_data), ]
```

## Make data into long format for plotting on line graph
```{r}

N_data_long <- N_data %>%
  pivot_longer(cols = `NO3 (mg/kg)`:`NH4 (mg/kg)`,
               names_to = "Nitrogen compound",
               values_to = "Concentration (mg/kg)")

head(N_data_long, 20)
```
## Calculate summary statistics
```{r}

summaryN <- N_data_long %>%
  group_by(Treatment, Weeks, `Nitrogen compound`) %>%
  summarise(
    meanConc = mean(`Concentration (mg/kg)`),
    sdConc = sd(`Concentration (mg/kg)`)
  )

summaryN
```

```{r}

summaryN[!complete.cases(summaryN), ]

```

## Make the plot (fig. 7)
```{r}


N_plot <- ggplot(summaryN, aes(x = factor(Weeks), y = meanConc, color = factor(`Nitrogen compound`), group = `Nitrogen compound`)) +
  geom_line() +
  geom_point() +
  facet_wrap(~Treatment) +
  theme(text = element_text(family = "Arial", size = 16), panel.grid.major = element_blank(), 
           panel.grid.minor = element_blank(), legend.title = element_text(size = 12), 
           legend.position = "bottom") +
  labs(x = "Time (weeks)", y = "Mean concentration (mg/kg)", shape = "Nitrogen form") +
  scale_color_discrete(labels = c("NH4", "NO2", "NO3"))

  

N_plot + scale_color_manual(values = c("#E69F00", "#009E73",  "#56B4E9"))
```

Save the plot as jpeg
```{r}

ggsave(filename = "Nitrogen-linegraph.jpeg", plot = N_plot,  width = 9, unit = "in")

```