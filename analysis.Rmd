...
pH_data$pH <- as.double(pH_data$pH)
```
...
pH_normality <- pH_data %>%
  group_by(Treatment, Weeks) %>%
  shapiro_test(pH)
data.frame(pH_normality)
...
```{r}
res.aov <- pH_data %>%
  anova_test(dv = pH, wid = Sample.ID,
  within = c(Treatment, Weeks))
...
longData <- tidyData %>%
  dplyr::select(-2) %>%
  pivot_longer(cols = c(pH:`K (mg/kg)`),
               names_to = "Compound",
               values_to = "Result")
...
```
```{r}
longData$Compound <- as.factor(longData$Compound)
summary(longData)
```
