...
Removing rows with NA's, except in Note column.
```{r}
...
```
Renaming columns so they're easier to write in plot functions.
```{r}
...
for(i in 6:ncol(up_to7)){
  ...
  temp_plot <- ggplot(up_to7, aes_string(x = "Treatment", y = up_to7[,i], fill="weeks")) +
          geom_boxplot(alpha = 0.5) +
          theme_bw(base_size = 15) +
